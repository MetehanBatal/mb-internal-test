...
let selectedPillowcaseUpsellSize = "king";

let pillowcaseUpsellProduct = {
	color : selectedColor,
	size : selectedPillowcaseUpsellSize,
}

var pillowcaseVariants = {
    standard: {
		//stone: { variantId: 39509018149014, price: 44, productTitle: 'White Pillowcase - Standard', productReviews: 100, imgUrl: 'images/pillowcase-white.png' },
        white: { variantId: 39509018214550, price: 44, productTitle: 'White Pillowcase - Standard', productReviews: 100, imgUrl: 'images/pillowcase-white.png' },
        //sky_blue: { variantId: 39509018280086, price: 44, productTitle: 'White Pillowcase - Standard', productReviews: 100, imgUrl: 'images/pillowcase-white.png' },
        navy_blue: { variantId: 42193748295830, price: 44, productTitle: 'Navy Pillowcase - Standard', productReviews: 90, imgUrl: 'images/pillowcase-navy.png' },
        sand: { variantId: 42193759502486, price: 44, productTitle: 'Sand Pillowcase - Standard', productReviews: 85, imgUrl: 'images/pillowcase-sand.png' },
        terracotta: { variantId: 42310607011990, price: 44, productTitle: 'Terracotta Pillowcase - Standard', productReviews: 88, imgUrl: 'images/pillowcase-terracotta.png' },
        charcoal: { variantId: 42310607437974, price: 44, productTitle: 'Charcoal Pillowcase - Standard', productReviews: 95, imgUrl: 'images/pillowcase-charcoal.png' },
        sage: { variantId: 42310607634582, price: 44, productTitle: 'Sage Pillowcase - Standard', productReviews: 87, imgUrl: 'images/pillowcase-sage.png' },
        rosewood: { variantId: 44267087233174, price: 44, productTitle: 'Rosewood Pillowcase - Standard', productReviews: 80, imgUrl: 'images/pillowcase-rosewood.png' },
        silver_grey: { variantId: 43880190115990, price: 44, productTitle: 'Silver Grey Pillowcase - Standard', productReviews: 92, imgUrl: 'images/pillowcase-silver-grey.png' },
        //slate_blue: { variantId: 43880184479894, price: 44, productTitle: 'Silver Grey Pillowcase - Standard', productReviews: 92, imgUrl: 'images/pillowcase-silver-grey.png' },
    },
    king: {
        stone: { variantId: 39509018345622, price: 49, productTitle: 'Stone Pillowcase - King', productReviews: 78, imgUrl: 'images/pillowcase-stone.png' },
        white: { variantId: 39509018411158, price: 49, productTitle: 'White Pillowcase - King', productReviews: 96, imgUrl: 'images/pillowcase-white.png' },
        sky_blue: { variantId: 39509018476694, price: 49, productTitle: 'Sky Blue Pillowcase - King', productReviews: 85, imgUrl: 'images/pillowcase-sky-blue.png' },
        navy_blue: { variantId: 42193831035030, price: 49, productTitle: 'Navy Pillowcase - King', productReviews: 91, imgUrl: 'images/pillowcase-navy.png' },
        sand: { variantId: 42193866588310, price: 49, productTitle: 'Sand Pillowcase - King', productReviews: 84, imgUrl: 'images/pillowcase-sand.png' },
        terracotta: { variantId: 42310608584854, price: 49, productTitle: 'Terracotta Pillowcase - King', productReviews: 89, imgUrl: 'images/pillowcase-terracotta.png' },
        charcoal: { variantId: 42310609371286, price: 49, productTitle: 'Charcoal Pillowcase - King', productReviews: 94, imgUrl: 'images/pillowcase-charcoal.png' },
        sage: { variantId: 42310609666198, price: 49, productTitle: 'Sage Pillowcase - King', productReviews: 86, imgUrl: 'images/pillowcase-sage.png' },
        rosewood: { variantId: 44267099979926, price: 49, productTitle: 'Rosewood Pillowcase - King', productReviews: 79, imgUrl: 'images/pillowcase-rosewood.png' },
        slate_blue: { variantId: 43880185299094, price: 49, productTitle: 'Slate Blue Pillowcase - King', productReviews: 83, imgUrl: 'images/pillowcase-slate-blue.png' },
        silver_grey: { variantId: 43880191295638, price: 49, productTitle: 'Silver Grey Pillowcase - King', productReviews: 93, imgUrl: 'images/pillowcase-silver-grey.png' },
    }
};

function updatePillowcaseUpsellUI(selectedColor) {
    const availableSizes = ['standard', 'king'];
    let selectedSize = null;

    // Check availability for the selected color
    for (let size of availableSizes) {
        if (pillowcaseVariants[size][selectedColor]) {
            selectedSize = size;
            break;
        }
    }

    // If no size is available for the selected color, default to white
    if (!selectedSize) {
        selectedColor = 'white';
        for (let size of availableSizes) {
            if (pillowcaseVariants[size][selectedColor]) {
    ...
    // If white is not available, default to stone
    if (!selectedSize) {
        selectedColor = 'stone';
        for (let size of availableSizes) {
            if (pillowcaseVariants[size][selectedColor]) {
    ...
    // If stone is not available, select a random color that is fully in stock
    if (!selectedSize) {
        for (let color in pillowcaseVariants['standard']) {
            if (pillowcaseVariants['standard'][color] && pillowcaseVariants['king'][color]) {
                selectedColor = color;
    ...
    // Update the UI with the selected pillowcase upsell
    document.querySelectorAll(".pillowcase-upsell-selector").forEach((selectorEl) => {
        const dataValue = selectorEl.getAttribute("data-value").toLowerCase();
        if (dataValue === selectedSize) {
        ...
        // Manage 'sold' class
        if (pillowcaseVariants[dataValue][selectedColor]) {
            selectorEl.classList.remove("sold");
        } else {
            selectorEl.classList.add("sold");
        }
    });

    document.querySelectorAll(".pillowcase-upsell-description-price").forEach((upsellPriceEl) => {
        upsellPriceEl.textContent = selectedSize === "king" ? "$49*" : "$44*";
    });
    document.querySelectorAll(".pillowcase-upsell-description-strikethrough").forEach((upsellPriceEl) => {
        upsellPriceEl.textContent = selectedSize === "king" ? "$69*" : "$61*";
    });

    document.querySelectorAll(".pillowcase-upsell-selected-size").forEach((upsellSelectedSize) => {
        upsellSelectedSize.textContent = selectedSize;
    });

    // Update the selected pillowcase upsell product
    pillowcaseUpsellProduct.color = selectedColor;
    selectedPillowcaseUpsellSize = selectedSize;
}

document.querySelectorAll(".pillowcase-upsell-selector").forEach((pillowcaseSelector) => {	

    pillowcaseSelector.addEventListener("click", (e) => {					
        document.querySelectorAll(".pillowcase-upsell-selector").forEach((el) => {
            el.classList.remove("active");
        });

        e.currentTarget.classList.add("active");		
        selectedPillowcaseUpsellSize = e.currentTarget.getAttribute("data-value");
        console.log("Selected Size:", selectedPillowcaseUpsellSize);		

        document.querySelectorAll(".pillowcase-upsell-description-price").forEach((upsellPriceEl) => {
            upsellPriceEl.textContent = selectedPillowcaseUpsellSize === "king" ? "$49*" : "$44*";
        });

        document.querySelectorAll(".pillowcase-upsell-selected-size").forEach((upsellSelectedSize) => {
            upsellSelectedSize.textContent = selectedPillowcaseUpsellSize;
        });		
    });	
});
...
function setFreeTowelImage(color) {
	...
	const pillowcaseUpsellImage = document.querySelectorAll(".pillowcase-upsell-image-desktop");
	freeImage.forEach((el) => {
		if (freeImage) {
			el.src = `/sheets/ksp/images/towels/towel_${color}.png`;
		}
	})
	pillowcaseUpsellImage.forEach((el) => {
		if (pillowcaseUpsellImage) {
			el.src = `/sheets/ksp-one-click/images/upsell-pillowcase/${color}.png`;
		}
	})
}
...
document.querySelectorAll(".color-choice").forEach(colorEl => {
	colorEl.addEventListener('click', function (e) {
		...
		const fabric = 'signature';
		console.log("Selected Color Inside", selectedColor);
		console.log("Selected Color Inside", color);
		document.querySelectorAll(".selected-color").forEach((upsellSelectedColorEl) => {
			upsellSelectedColorEl.textContent = color.replace("_", " ");
		});
	
		let out_of_stockPillowcases = ["stone", "sky_blue", "slate_blue"];

		console.log("Selected Color", selectedColor)
		if(out_of_stockPillowcases.includes(selectedColor)) {
			console.log("Pillowcase is OOS")
		}
		...
		if (this.classList.contains("sold")) {			
			...
			if (availableSizes.length > 0) {
				...
				selVariant();
				updatePillowcaseUpsellUI(color)
			} else {
...
function updateSlider(color) {
	...
	const BASE_PATH = '/sheets/ksp/images/sliders';
...
function getUrlParams() {
	...
	return {
		...
		color: params.get('color')?.toLowerCase()?.replace(/\s+/g, '_')
	};
}
function initializeProductFromUrl() {
	const { size, color } = getUrlParams();
	const fabric = 'signature';
	const isMobile = window.matchMedia('(max-width: 480px)').matches;
	console.log("size", size, "color", color);
	if (!size && !color) return;
	if (size) {
		const isValidSize = checkStock(fabric, size, color || getDefaultColor());
		if (isValidSize) {
			selectSize(size, isMobile);
		}
	}
	if (color) {
		const currentSize = size || getCurrentSize();
		const isValidColor = checkStock(fabric, currentSize, color);

		if (isValidColor) {
			const colorElement = document.querySelector(
				`.${isMobile ? 'mobile' : 'desktop'}-colors-container .color-choice[data-color="${color}"]`
			);

			if (colorElement && !colorElement.classList.contains('sold')) {
				colorElement.click();
			}
		}
	}
...
initializeProductFromUrl();
function getDefaultColor() {
	const isMobile = window.matchMedia('(max-width: 480px)').matches;
	const firstAvailableColor = document.querySelector(
		`.${isMobile ? 'mobile' : 'desktop'}-colors-container .color-choice:not(.sold)`
	);
	return firstAvailableColor?.getAttribute('data-color')?.toLowerCase()?.replace(/\s+/g, '_');
}
function getCurrentSize() {
	const isMobile = window.matchMedia('(max-width: 480px)').matches;
	const currentSizeElement = isMobile
		? document.querySelector('.mobile-size-selector .mobile-size-item.current-size')
		: document.querySelector('.size-sc .list li.option.current-size');

	return isMobile
		? currentSizeElement?.getAttribute('value')?.toLowerCase()
		: currentSizeElement?.getAttribute('data-value')?.toLowerCase();
}
...
document.querySelectorAll(".buy-now-button").forEach(function (atcButton) {
	atcButton.addEventListener('click', function (e) {
		...
		clearCart().then(x => {
			...
			const isPillowcaseUpsellChecked =
					document.querySelector("#pillowcase-upsell-check-desktop")?.checked ||
					document.querySelector("#pillowcase-upsell-check-mobile")?.checked;
			if(isPillowcaseUpsellChecked) {
				console.log("Pillowcase Variants : ", pillowcaseVariants)
				console.log("selectedPillowcaseUpsellSize : ", selectedPillowcaseUpsellSize)
				console.log("selectedColor :", selectedColor)
				data[pillowcaseVariants[selectedPillowcaseUpsellSize][selectedColor]['variantId']] = 1;
			}
