document.addEventListener('DOMContentLoaded', function() {
	...
	window.checkoutReadyCallbacks.push(() => {
		console.log("CheckoutReadyCallbacks Started!")
		if (checkoutData.cart.shippingZone === "CA") {			
			isCanadian = true;
			console.log("Is Canadian : ", isCanadian)
		}	
	})
	...
	*/
	...
	function setFreeTowelImage(color) {
		...
		freeImage.forEach((el) => {
			if (freeImage) {
				el.src = `/sheets/ksp-ca/images/towels/towel_${color}.png`;
			}
		})
	}
	...
	function updateSlider(color) {
		...
		const BASE_PATH = '/sheets/ksp-ca/images/sliders';
			...
			clearCart().then(x => {
				...
				window.checkout.apiClient.setVariantQuantities(data).then(x => {
					...
					if (hasAnyTargetVariant) {
						if(isCanadian === true) {
							window.location.href = '/sheets/checkout-ca/index.html?uf=red';
						}else {
							window.location.href = '/sheets/checkout/index.html?uf=red';
						}						
					} else {
						if(isCanadian === true) {
							window.location.href = '/sheets/checkout-ca/index.html?uf=green';
						}else {
