...
function setFreeTowelImage(color) {
	...
	freeImage.forEach((el) => {
		if (freeImage) {
			el.src = `/sheets/ksp-towel-test/images/towels/towel_${color}.png`; // Will change on live
		}
	})
}
...
function updateSlider(color) {
	...
	const BASE_PATH = '/sheets/ksp-towel-test/images/sliders'; // Will change on live 
...
function preloadImages(color) {
	const BASE_PATH = '/sheets/ksp-towel-test/images/sliders'; // Will Change
	const reqSliderCount = imageCount[color];
	const imagePromises = [];
	const imageObjects = [];
		...
		clearCart().then(x => {
			...
			if (isMobile) {
				...
				if (isFreeGiftChecked) {
					...
					const whiteColors = ["white", "sage", "terracotta", "silver_grey", "sky_blue",  "rosewood"];
					const stoneColors = ["stone", "charcoal", "sand", "slate_blue", "navy_blue"];
