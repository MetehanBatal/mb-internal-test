jQuery(document).ready(function ($) {
	...
	var variants = {
		signature: {
			king: {
				/*
				stone: { variantId: 44198190284950, productTitle: 'Comforter King/Cali King - Stone + Sage Green Sheet Set', price: 159, regularPrice: 298, productSavings: 47, productReviews: 234, imgUrl: 'images/3pc-stone.png' },
				*/
				white: { variantId: 44198189531286, productTitle: 'Comforter King/Cali King - White + Sage Green Sheet Set', price: 159, regularPrice: 298, productSavings: 47, productReviews: 234, imgUrl: 'images/3pc-white.png' },
			},
			queen: {
				// stone: { variantId: 44198190252182, productTitle: 'Comforter Full/Queen - Stone + Sky Blue Sheet Set', price: 149, regularPrice: 278, productSavings: 46, productReviews: 234, imgUrl: 'images/3pc-stone.png' },
				// white: { variantId: 44198189498518, productTitle: 'Comforter Full/Queen - White + Sky Blue Sheet set', price: 149, regularPrice: 278, productSavings: 46, productReviews: 234, imgUrl: 'images/3pc-white.png' },
			},
		}
	};
	...
	*/
	...
	function setFreeOfferImage(color) {
		var size = $('.current-size[data-value]').attr("data-value");
		
		if(size === "king") {
			$(".free-product").attr("src", `images/king-free.png`)
		}else {
			$(".free-product").attr("src", `images/queen-free.png`)
		}
	...
	function updateSlider(color) {	
		...
		for (let i = 1; i < imageCount[color] + 1; i++) {
			...
			if(size === "queen") {
				mainSlideTemplate = `<div class="swiper-slide"><img src="/sheets/regular-comforter-with-free-sheet/images/sliders/queen_${color.toLowerCase().replace(' ', '-')}/0${i}.webp" alt="" class="product-img"></div>`;
				thumbnailsSlideTemplate = `<div class="swiper-slide product-icon-2"><img src="/sheets/regular-comforter-with-free-sheet/images/sliders/queen_${color.toLowerCase().replace(' ', '-')}/0${i}_thumb.webp" loading="lazy" alt="" class="img-fluid"></div>`;
			}else {
				mainSlideTemplate = `<div class="swiper-slide"><img src="/sheets/regular-comforter-with-free-sheet/images/sliders/${color.toLowerCase().replace(' ', '-')}/0${i}.webp" alt="" class="product-img"></div>`;
				thumbnailsSlideTemplate = `<div class="swiper-slide product-icon-2"><img src="/sheets/regular-comforter-with-free-sheet/images/sliders/${color.toLowerCase().replace(' ', '-')}/0${i}_thumb.webp" loading="lazy" alt="" class="img-fluid"></div>`;
			}
		...
		$(".buy-now-button").click(function (e) {
			...
			clearCart().then(x => {
				...
				window.checkout.apiClient.setVariantQuantities(data).then(x => {
					...
					window.location.href = '/checkout_bf/secure.html';
				});
			});
		});
	});
	...
	$(".list li.option:last-child").addClass("sold");
});
