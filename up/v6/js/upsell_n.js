...
let upsellHench = {
    ...
    initVariantSelection: function () {
		...
		if (hasSelector) {
		  ...
		  self.choices.passedElement.element.addEventListener(
			"choice",
			function (event) {
			  if (pagePath === "5b" || pagePath === "5b-cooling" || pagePath === "5b-3temp") {
	...
	updateVariant: function (from) {
		...
		window.checkoutReadyCallbacks.push(() => {
			...
			const shouldUpdatePrice =
				pagePath === "3b" || pagePath === "3b-v1" || pagePath === "5b" || pagePath === "5b-cooling" || pagePath === "5b-3temp";
	...
	filterOutOfStockProducts: function (from) {
		...
		if (from === 'color') {
			...
			colors.forEach(function (color) {
				...
				} else if (pagePath === "5b" || pagePath === "5b-cooling" || pagePath === "5b-3temp") { 
	...
	swapVariantColors: function () {
		...
		if (hasDynamicColorChange) {
			...
			const hasRosewoodColor = rosewoodProducts.some(variant => cart.includes(variant));
			...
			if (hasRosewoodColor) { self.color = "rosewood" };
