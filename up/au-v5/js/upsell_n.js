...
let upsellHench = {
    ...
    initVariantSelection: function () {
		...
		if (hasSelector) {
		  ...
		  if (pagePath === "5b" || pagePath === "5b-cooling") {
			self.quantity = nextAvailableSize === "queen" ? 2 : 1;
		  } else {
	...
	updateQuantities: function () {
		...
		Object.keys(variants).forEach(function (quantity, index) {
			...
			if (pagePath === "5b" || pagePath === "5b-cooling") {
				...
				label = `White - Queen - ${usPrefix} $${variant ? variant["price"] : "132"
					}* `;
				if (index === 0) {
					label = `White - King/Cali King - ${usPrefix} $${variant ? variant["price"]?.toFixed(2) : "139.30"
						}* `;
				}
			} else if (pagePath === "4b") {
	...
	updateSliderImages: function () {
		...
		images.forEach(function (image) {
			...
			let imagePath;
			if(pagePath === "2d") {
				imagePath = `/up/au-v5/images/sliders/${pagePath}/new/${self.color}/0${index}.webp`;
				...
				imagePath = `/up/au-v5/images/sliders/${pagePath}/${self.color}/0${index}.webp`;

				if (image.querySelector("img").classList.contains("thumb-img")) {
					imagePath = `/up/au-v5/images/sliders/${pagePath}/${self.color}/0${index}_thumb.webp`;
				}
