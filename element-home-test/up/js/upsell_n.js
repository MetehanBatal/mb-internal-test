...
let upsellHench = {
	...
	initVariantSelection: function() {
		...
		if (hasSelector) {
			...
			self.choices.passedElement.element.addEventListener( 'choice', function(event) {
				console.log(event);
				document.querySelector('.upsell-slider img').setAttribute('src', `./images/sliders/${pagePath}/${event.detail.choice.id}.jpg`)
				// self.quantity = event.detail.choice.id;

				document.querySelector('[name="upsellParam1"]').value = event.detail.choice.value;
				// self.updateVariant();
	...
	updateQuantities: function() {
		...
		Object.keys(variants).forEach(function(quantity, index) {
			let label = `Add ${quantity} Sheets Sets for${usPrefix} $${(variants[quantity][self.fabric][self.size][self.color]['sale_price']  / quantity ).toFixed(2)}* each (${discounts[index]}  OFF)`;
			let selected = false;
			if (index === 0) {
				let label = `Add ${quantity} Sheet Sets for${usPrefix} $${(variants[quantity][self.fabric][self.size][self.color]['sale_price']  / quantity ).toFixed(2)}* each (${discounts[index]}  OFF)`;
			}
	...
	updateSliderImages: function() {
		...
		images.forEach(function(image) {
			if (index > images.length / 2) {
				index = 1; }
			let imagePath = `./images/sliders/${pagePath}/${self.color}/${index}.jpg`;

			if(image.querySelector("img").classList.contains("thumb-img")) {
				imagePath = `./images/sliders/${pagePath}/${self.color}/${index}_.jpg`;}
	...
	choiceSelect: function() {
		if(hasChoicesSelector) {
			document.querySelectorAll(".variants option").forEach(opt => {
				console.log(opt)
